...
/**
 * A hash map implementation of that uses open addressing for keys. To minimize
 * the memory footprint, this class uses open addressing rather than chaining.
 * Collisions are resolved using linear probing. Deletions implement compaction,
 * so cost of remove can approach O(N) for full maps, which makes a small
 * loadFactor recommended.
 *
